...
import static com.cube.util.Constants.repeate;
...
public class Cube {
	 ...
	 */
	private Face[] faces = new Face[MAX_FACES];
	/**
	...
	private void print(PrintWriter out, String[] piece) {
		for (int i = 0; i < MAX_CELLS; i++) {
			out.print(SPACER);
			out.print(piece[i]);
			out.println(SPACER);
	...
	/**
	 * Print the Test CubeSolution.
	 * 
	 * @param outVar
	 */
	public void printTest(PrintStream outVar) {
		if (!isCompleted()) {
			return;
		}
		...
		final String[] piece2 = faces[2].getPiece().getTextRepresentation();
		for (int i = 0; i < MAX_CELLS; i++) {
			outVar.print(piece0[i]);
			outVar.print(piece1[i]);
			outVar.println(piece2[i]);
		}
		
		printTest(outVar, faces[3].getPiece().getTextRepresentation());
		printTest(outVar, faces[4].getPiece().getTextRepresentation());
		printTest(outVar, faces[5].getPiece().getTextRepresentation());
	}
	
	private void printTest(PrintStream outVar, String[] piece) {
		for (int i = 0; i < MAX_CELLS; i++) {
			outVar.print(SPACER);
			outVar.print(piece[i]);
			outVar.println(SPACER);
		}
